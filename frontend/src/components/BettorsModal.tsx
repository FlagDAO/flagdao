...
const BettorsModal: React.FC<HoverModalProps> = ({ bettors_plg, flag_id }) => {
  ...
  const { data, isError, isLoading, refetch, isFetching, error } =
    useContractRead({
    ...
    <div className="relative inline-block">
      ...
      >
        <div className="bg-slate-50 rounded-lg shadow-lg w-2/5 py-10 px-10 relative">
          ...
          <button
            onClick={closeModal}
            className="absolute top-2 left-2 bg-gradient-to-r from-red-200 via-red-300 to-yellow-200 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-red-100 rounded-lg text-white font-bold py-1 px-2"
          >
            X
          </button>
